...
# editing a file. This repo carried a hand-set `python-3.11+` badge that was
# already wrong against `requires-python = ">=3.9"` in pyproject.toml, and no
# Coverage badge at all against an enforced 92% floor.
#
...
# own.
#
# Runs from .github/workflows/test.yml on pushes to main only, from the
# python-3.12 leg of the pytest matrix. The `badges` branch is seeded before
# this script is ever invoked (alpha-engine-config-I10434), so the contents
# API is enough and no branch is ever created here.
set -euo pipefail
