...
via the CLI entry (``python -m nousergon_lib.alerts publish ...``) —
mirrors the :mod:`nousergon_lib.transparency` ``--cadence daily/weekly``
CLI convention.
...
- CLI: ``python -m nousergon_lib.alerts publish --message "..."
...
# Mirrors the :mod:`nousergon_lib.transparency` ``python -m`` pattern so
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m nousergon_lib.alerts",
        description=(
