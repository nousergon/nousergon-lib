...
- CLI: ``python -m nousergon_lib.ssm_log_capture run --slug <X>
...
- S3 upload failures (boto3 ``ClientError``, missing creds, missing
  ...
  would mask it. Matches :mod:`nousergon_lib.alerts`' fail-safe
  posture.
"""
...
def run(
    ...
    *,
    ...
    try:
        ...
        msg = f"nousergon_lib.ssm_log_capture: cannot exec {cmd!r}: {exc}\n"
        _append_log(log_path, msg)
        print(msg, file=sys.stderr)
        exit_code = 127
    except Exception as exc:
        msg = f"nousergon_lib.ssm_log_capture: subprocess setup failed: {type(exc).__name__}: {exc}\n"
        _append_log(log_path, msg)
        print(msg, file=sys.stderr)
        exit_code = 127
    finally:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m nousergon_lib.ssm_log_capture",
        description=(
